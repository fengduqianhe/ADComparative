...
class AdniDataSet_train(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img2 = nibabel.load(self.img_path + str(self.subjects2['index'][
                                                    idx]) + '.nii')
        score1 = str(self.subjects1['score'][idx])
        score2 = str(self.subjects2['score'][idx])
        ...
        if math.fabs(float(score1)*30-float(score2)*30) <= 3:
            label3 = 1
        elif float(score1)*30 - float(score2)*30 < -3:
            label3 = 0
        else:
            label3 = 2
        ...
        return img_array1, img_array2, label3
